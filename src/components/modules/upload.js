...
;(function ($) {
	...
	window.onload = () => {
		...
		let ulElement = document.getElementById('vf-upload__filelist')
		let upToSize = document.getElementById('up-to-size')
		let uploadButtonLabel = document.querySelector(
			'#vf-upload__container > label'
		)
		let uploadForm = document.getElementById('vf-upload__container')
		let fileArray = []

		if (uploadForm && uploadForm.classList.contains('vf-upload__container')) {
			document
				.getElementById('vf-upload__button')
