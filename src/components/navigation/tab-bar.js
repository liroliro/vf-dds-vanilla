// const docs = document.getElementsByClassName('vf-tab-bar-item')
// const tabs = document.getElementsByClassName('vf-panel')
// tabs[0].style.display = 'block'

// Array.from(docs).map((el) => {
// 	el.addEventListener('click', () => {
// 		const relatedTabs = Array.from(tabs).filter(
// 			({ ariaLabel }) => ariaLabel === el.id
// 		)

// 		setActiveTab(el, relatedTabs[0])
// 	})
// })

// const setActiveTab = (tab, content) => {
// 	Array.from(tabs).forEach((el) => (el.style.display = 'none'))
// 	Array.from(docs).forEach((el) =>
// 		el.classList.remove('vf-tab-bar-item--active')
// 	)
// 	tab.classList.toggle('vf-tab-bar-item--active')
